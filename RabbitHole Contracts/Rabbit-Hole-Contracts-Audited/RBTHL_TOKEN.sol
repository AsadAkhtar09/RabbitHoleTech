...
contract RabbitHoleToken is ERC20, Ownable, ReentrancyGuard {
    ...
    function salePriceEth() public view returns (uint256) {
        ...
        uint256 rbthlpriceInEth = (salePriceUsd.mul(10**18)).div(
            uint256(ethPriceUsd).mul(10**10)
        );

        return rbthlpriceInEth;
    }

    function computeTokensAmount(uint256 funds)
        public
        view
        returns (uint256, uint256)
    {
        uint256 salePrice = salePriceEth();
        uint256 tokensToBuy = (funds.div(salePrice)).mul(10**18);
    ...
    function giveTokens(address _account, uint256 _amount) external nonReentrant {
        require(isAdmin[_account] == true, "No Admin Found");
        _mint(_account, _amount);
    }

    function sendEth(address payable _to) public payable nonReentrant {
